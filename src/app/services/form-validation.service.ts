...
export class FormValidationService {
  ...
  formatCurrencybbps(value, formGroup: FormGroup, currency?: any) {
    if (value == '0') {
      if (formGroup.contains('amt')) formGroup.get('amt').reset();
      return;
    }
    if (value != '') {
      let updatedCurrency = this.customCurrencyPipe.transform(value.trim(), 'decimal');
      if (updatedCurrency.trim().replace(/[^.0-9]+/g, '') == ' 0.00') {
        if (formGroup.contains('amt')) formGroup.get('amouamtnt').reset();
      } else {
        if (updatedCurrency.trim().replace(/[^.0-9]+/g, '') == '0') {
          formGroup.get('amt').reset()
        } else {
          console.log(updatedCurrency);
          formGroup.patchValue({ amt: updatedCurrency });
          //formGroup.patchValue({ amount: updatedCurrency });
  ...
  formatCurrencyGeneric(value, formGroup: FormGroup, fld) {
    if (value == '0') {
